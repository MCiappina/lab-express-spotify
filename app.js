...
app.get("/artist-search", async (request, response) => {
    try {
        let spotifyResponse = await spotifyApi.searchArtists(request.query.artistName);
        response.render("artist-search-results", {data: spotifyResponse.body.artists.items});
//    
